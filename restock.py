...
from restock_invoice import generate_restock_invoice
...
# Main function to process restock transaction
def process_restock(medicines):
    ...
    print("=" * 150)
    for item in order:
        print(f"  {item['name']:<25} {item['quantity']} {item['unit_type']}(s)"
              f"  Rs {item['total']:>8.2f}")
    print("-" * 150)
