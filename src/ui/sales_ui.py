...
from src.core.invoices import generate_sale_invoice
...
def process_sale(medicines):
    ...
    while True:
        ...
        if tablets_needed > medicine["stock"]:
            ...
            print(f"      Available: {medicine['stock']} tablets ({available_strips} full strips)")
            continue
        ...
        if unit_type == "tablet" and result["strips"] >= 1:
            print(f"  Breakdown   : {result['strips']} strip(s) + {result['remaining_tablets']} tablet(s)")
