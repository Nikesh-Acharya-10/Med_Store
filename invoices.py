#import os , datetime for handling file operations and timestamps.
import os
from datetime import datetime

INVOICES_DIR = "invoices"

#` This file contains functions to generate and save invoices for sales and restocks. It ensures that invoices are stored in a dedicated directory with unique filenames based on timestamps. The invoices are formatted to include all relevant details such as customer/supplier information, itemized lists of medicines, totals, and timestamps for record-keeping.`
...
# The `generate_sale_invoice` function creates a formatted invoice for a sale transaction, while the `generate_restock_invoice` function does the same for restock transactions.
#  Both functions save the generated invoice to a file and return the filename for reference.
def generate_sale_invoice(customer_name, items, grand_total):
    ...
    for item in items:
        ...
        # Build a readable quantity string
        if unit_type == "tablet":
            if item["strips"] >= 1:
...
#This function helps to generate a restock invoice with the details that is entered by the user .
